...
//        height = min(height * 0.9, naturalHeight);
          width = min(width * 0.8, naturalWidth);
          height = min(height * 0.8, naturalHeight);
        image = {
  ...
  $.extend(prototype, {
    ...
    prev: function () {
      ...
      // View the next image
      next: function () {
        this.view(min(this.index + 1, this.length));
        if((this.index + 1) == (this.length)){
      	  layer.msg('这是最后一张了！',{time: 800});
        }
  	  //alert("22");
      },
...
//        	this.$tooltip.text(10000%);
  ...
  Viewer.TEMPLATE = (
    ...
    '   style="position:absolute; visibility:inherit; top:0px;left:0px;width:100%;' +
     '   height:100%;z-index:-1; opacity:0px;filter:alpha(opacity=0);"></iframe>' +
      '<div class="viewer-canvas"></div>' +
      '<div class="viewer-footer">' +
        '<div class="viewer-title"></div>' +
        '<ul class="viewer-toolbar" style="width: 90px;">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="viewer-prev" data-action="prev" style="width:49px;height:49px;position:absolute;left:20px;top:50%;cursor: pointer;" title="查看上一张"></div>'+
      '<div class="viewer-next" data-action="next" style="width:49px;height:49px;position:absolute;right:20px;top:50%;cursor: pointer;" title="查看下一张"></div>'+
